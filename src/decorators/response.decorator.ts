...
const buildHttpDecorator = (
  ...
  return (_, __, descriptor: PropertyDescriptor) => {
    ...
    if (successCode) {
      SetMetadata(
        META.HTTP_SUCCESS_RESPONSE_CODE,
        successCode,
      )(descriptor.value);
    }
    if (errorMsg) {
    ...
    if (successMsg) {
      SetMetadata(
        META.HTTP_SUCCESS_RESPONSE_MESSAGE,
        successMsg,
      )(descriptor.value);
    }
    if (errorStatus) {
      SetMetadata(
        META.HTTP_ERROR_RESPONSE_STATUS,
        errorStatus,
      )(descriptor.value);
    }
    if (successStatus) {
      SetMetadata(
        META.HTTP_SUCCESS_RESPONSE_STATUS,
        successStatus,
      )(descriptor.value);
    }
    if (usePaginate) {
      SetMetadata(
        META.HTTP_RESPONSE_TRANSFORM_PAGINATE,
        true,
      )(descriptor.value);
    }
    return descriptor;
...
export function handle(...args: any[]) {
  ...
  const errorCode: number = isOption(option)
    ? option.errorCode
    : DEFAULT_ERROR.code;
  ...
  const usePaginate: boolean = isOption(option)
    ? option.usePaginate || false
    : false;
  return buildHttpDecorator({
    errorCode,
    successCode,
